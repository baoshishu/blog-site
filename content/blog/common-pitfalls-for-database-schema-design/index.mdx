...
假设我们需要对一个集团公司员工的工资进行统计，工资包括固定薪酬 Salary, 补贴 Subsidy，另外员工需要交税 Tax, 员工的收入可以通过公示 `Income = Salary + Subsidy - Tax` 计算得出。
...
class Order(Base):
    ...
    def address(sef, address:Address):
        self._address = dataclass.asdict(address)
```
...
如果你发现代码里面有下面这些 Code Smell， 你可能需要考虑抽象出对应的*Business Model*
...
1. 不同的 Model 之间互相依赖，这是一个需要一个*Business Model*来协调这些互相依赖的 Mode 的信号。

抽象出*Business Model*的好处有

1. 业务和数据库解耦，如果把数据库作为 implementation detail，后续的重构不需要修改业务层的代码
1. 通过对*ORM Model*的封装，把一些复杂的查询抽到一个 namespace 里面统一维护
1. 业务边界（Boundary）更加清晰，代码易于维护。
1. 更利于编写测试，测试 _Business Model_ 比 _ORM Model_ 更有意义也更方便。

_Business Model_ 可以是一个类（class）， 也可以是一个模块（module）。并不是说有了 _Business Modal_，我们就完全不能直接操作 _ORM Model_ 了，简单的 CRUD 操作往往也没有必要引入额外的 layer, 什么时候用 _Business Model_ 也没有金科玉律，需要根据具体业务场景做分析判断。

引入 _Business Model_ 还有一个好处就是，如果已经存在一个比较庞大或者设计不太好的 _ORM Model_ 时，可以通过 _Business Model_ 把这些 **dirty** 的部分集中起来，暴露一个简单 API 给其他业务使用。这个时候 _Business Model_ 就相当与一个 [facade](https://en.wikipedia.org/wiki/Facade)。
